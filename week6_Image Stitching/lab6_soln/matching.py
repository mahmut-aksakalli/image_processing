		...
		if edit == False:
			if point == False:
				if(i%2 == 0):
					...
					cv2.circle(result,(x,y),9,(255,255,0),2)
				else:
					if(no == img_no):
						del coord_list[-1]
						i-=1
			...
			if(no == img_no):
				...
				while(j<len(coord_list)-1):
					...
					cv2.circle(result,coord_list[j],9,(255,255,0),2)
					cv2.circle(result,coord_list[j+1],1,(255,0,0),2)
					cv2.circle(result,coord_list[j+1],9,(255,255,0),2)
...
def match(fname1,fname2):
	global coord_list,edit,ix,iy,i,h,w,result
